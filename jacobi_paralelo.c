...
void jacobi(double **A, double *B, double p) {
    ...
    int iteracoes = 0; // ADICIONADO: Declara e inicializa o contador
    ...
    while(erro > p) {
        ...
        iteracoes++; // ADICIONADO: Incrementa o contador a cada iteração
    }
    clock_gettime(CLOCK_MONOTONIC, &fim);

    printf("\nConvergência alcançada em %d iterações.\n", iteracoes); // ADICIONADO: Imprime o total de iterações
